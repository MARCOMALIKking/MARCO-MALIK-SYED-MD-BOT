...
// ── Auto session restore from SESSION_ID ──────────────────────────────────────
(function restoreSessionOnStartup() {
    ...
    if (!sessionId) return;
    const b64 = sessionId.includes('~') ? sessionId.split('~').slice(1).join('~') : sessionId;
    if (!b64) return;
    try {
        ...
        console.log('⚠️  Could not restore session from SESSION_ID:', e.message);
    }
})();
...
global.channelLink = "https://whatsapp.com/channel/0029Vb2RnBlHgZWeOydeRV1H";
global.ytch = "MARCO MALIK";
...
const channelInfo = {
    contextInfo: {
        ...
        forwardedNewsletterMessageInfo: {
            newsletterJid: '120363369769257255@newsletter,
            newsletterName: 'MARCO MĀLÏK',
            serverMessageId: -1
        }
    }
};
        ...
        if (message.message?.buttonsResponseMessage) {
            ...
            if (buttonId === 'channel') {
                await sock.sendMessage(chatId, {
                    text: '📢 *Join our Channel:*\nhttps://whatsapp.com/channel/0029Vb2RnBlHgZWeOydeRV1H'
                }, { quoted: message });
                return;
                ...
                await sock.sendMessage(chatId, {
                    text: `🔗 *Support*\n\nhttps://chat.whatsapp.com/G8HrhPiyp2y8dhsfPUToRs?mode=gi_t`
                }, { quoted: message });
                return;
            }
        }
